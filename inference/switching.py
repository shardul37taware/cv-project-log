...
model_shape = YOLO(r"D:\git\cv-project-log\target\models\ultra_final_final.pt")
...
while True:
    ...
    results = model_shape.predict(source=frame, show=False, conf=0.8, verbose=False)
    result = results[0]
    ...
    for tracking in TrackResults:
        ...
        if roi[0] < cx < roi[2] and roi[1] < cy < roi[3] and track_id not in log_df['id'].values:
            ...
            log_df.loc[len(log_df)] = [
                track_id, x1, y1, x2, y2, model_shape.names[cls], round(conf, 2), time_tracked
            ]
