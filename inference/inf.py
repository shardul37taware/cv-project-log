...
while True:
    ...
    results = model.predict(source=frameEq, show=False, conf=0.8, verbose=False)
    result = results[0]
    ...
    for tracking in TrackResults:
        ...
        cv2.rectangle(frameEq, (x1, y1), (x2, y2), (255, 0, 0), 3)
        cv2.putText(frameEq, f'{track_id}', (x1, y1 - 10),
...
# Save logs
log_df.to_csv("test2.csv", index=False)
